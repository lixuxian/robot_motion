...
public:
    ...
    // matrix*scalar
    friend inline mat operator * (const mat& lhs, pTYPE v) {
        return helpers::doMul(lhs, v);
    }

    // scalar*matrix
    friend inline mat operator * (pTYPE v, const mat& rhs) {
...
inline int     transpose(int v)    { return v; }
inline float   transpose(float v)  { return v; }
inline double  transpose(double v) { return v; }

// Transpose a matrix
template <typename TYPE, size_t C, size_t R>
mat<TYPE, R, C> transpose(const mat<TYPE, C, R>& m) {
    mat<TYPE, R, C> r;
...
template <
    ...
    size_t SIZE
>
mat<TYPE, SIZE, 1> transpose(const VEC<TYPE, SIZE>& v) {
    mat<TYPE, SIZE, 1> r;
...
mat<T, N, N> invert(const mat<T, N, N>& src) {
    T t;
    size_t swap;
    mat<T, N, N> tmp(src);
    mat<T, N, N> inverse(1);
