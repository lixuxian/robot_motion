...
template <
    ...
    size_t SIZE
>
inline VLHS<TYPE, SIZE> operator + (
        const VLHS<TYPE, SIZE>& lhs,
        const VRHS<TYPE, SIZE>& rhs) {
...
template <
    ...
    size_t SIZE
>
TYPE length(const V<TYPE, SIZE>& v) {
...
template <
    ...
    size_t SIZE
>
TYPE length_squared(const V<TYPE, SIZE>& v) {
...
template <
    ...
    size_t SIZE
>
V<TYPE, SIZE> normalize(const V<TYPE, SIZE>& v) {
    return v * (1/length(v));
}
...
template <typename TYPE, size_t SIZE>
vec<TYPE, SIZE> operator - (const vec<TYPE, SIZE>& lhs) {
    vec<TYPE, SIZE> r;
    for (size_t i=0 ; i<SIZE ; i++)
        r[i] = -lhs[i];
    return r;
...
public:
    ...
    friend inline vec operator * (const vec& lhs, pTYPE v) {
        return helpers::doMulScalar(lhs, v);
    }
    friend inline vec operator * (pTYPE v, const vec& rhs) {
        return helpers::doScalarMul(v, rhs);
    }
    // friend inline TYPE dot_product(const vec& lhs, const vec& rhs) {
